...
void init_pid_controller( PIDController* pid_controller, float KP, float KI, float KD){
    ...
    pid_controller->dt = 0.004; // Need to test loop to see if we need a more accurate re-calc between function calls
    pid_controller->min_output = MIN_PULSE_US;
...
float calculate_pid( PIDController* pid_controller, float error){
    ...
    } else if (output < pid_controller->min_output){
        output = pid_controller->min_output;
    }
